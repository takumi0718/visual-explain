...
def render_slope(section: CanonicalSection, definition) -> RenderResult:
    ...
    highlight_id = slope.highlight_id
    ...
    for index, item in enumerate(slope.items, start=1):
        ...
        item_blocks.append(
            ...
            f'<line class="ve-slope-item {tone_cls}"'
            f' x1="{_FROM_X}" y1="{y1}" x2="{_TO_X}" y2="{y2}">{emphasis_html}</line>'
            f'<circle class="ve-slope-endpoint {tone_cls}" cx="{_FROM_X}" cy="{y1}" r="4"></circle>'
            f'<circle class="ve-slope-endpoint {tone_cls}" cx="{_TO_X}" cy="{y2}" r="4"></circle>'
            f'<text class="ve-slope-value ve-slope-value-from" x="{_FROM_X}" y="{y1 - 8}"'
            f' text-anchor="end">{_esc(item.from_value_text)}</text>'
            f'<text class="ve-slope-value ve-slope-value-to" x="{_TO_LABEL_X}" y="{y2 - 8}"'
            f' text-anchor="start">{_esc(item.to_value_text)} {_esc(item.label)}</text>'
            f'</g>'
        )
    ...
    markup = (
        ...
        f'<p class="ve-fig-unit">単位: {_esc(slope.unit_label)}</p>'
