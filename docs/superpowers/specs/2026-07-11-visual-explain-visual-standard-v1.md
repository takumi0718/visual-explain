...
- **3段階読解パス**: 3秒＝第一画面（h1タイトル＋サブタイトル（あなたが決めること）＋条件最大2件）／30秒＝アクションタイトル＋主要証拠の拾い読み／3分＝本文根拠・確度・出所・deep-dive。既存の first-screen 契約と deep-dive 構造を継承する。（改訂 2026-07-11: ユーザー要望によりタイトル/サブタイトルを第一画面契約へ追加）
...
- **タイポグラフィ**: システムフォント単一族。階層はサイズ＋ウェイトのみ。5段タイプスケールを skeleton に固定する: 第一画面主張 30px/700・セクション主張 20px/700・本文 16px/400・図解コンテンツ 14px/400・補助 13px/400（rem 基準で実装し、この5値は実装時に変更しない）。本文の行長は日本語約40字を上限とする。行間は本文 1.7〜1.8・見出し 1.4〜1.5 をトークン化し、段落間隔もスケールから取る。（改訂 2026-07-11: ユーザー指摘「図解の文字を小さくし余白を残す」により図解コンテンツ用 --fs-figure 14px を追加し5段へ変更）
- **単一幅と整列**: コンテンツは単一の最大幅カラム（720px = narrative 幅）に収める。見出し・本文・図・表・キャプションを含む全要素が左右のエッジを共有し、中央寄せは第一画面の演出にも使わない。多列 matrix・分岐 flow・注釈付き比較図など密度の高い証拠は、ページ全体を広げず各自の横スクロールコンテナ内で溢れさせる。狭幅（モバイル）でも同じ規律を維持する。文章の可読幅とコンサル的一覧性を両立させる。（改訂 2026-07-11: デザインゲート後のユーザー指摘「見出し幅と図解幅のアンバランス」により、二段階幅を廃し単一幅へ変更）
