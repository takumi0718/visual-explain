...
- **受理条件:**
  ...
  - `blockContent: "number"` → 番号はレンダラが 1..n を採番（IR に番号を書かない）。全 item に `title`（30 字以内）必須。番号は識別子であり、`description` はコンセプトの代わりにならない。違反は `enumeration_structure_violation`。
...
- **表現:** `label` または番号＋`title` だけをコンセプト図形内に置く。`description` がある場合、縦（list）は図形の右、横（columns）は図形の下に図形外の説明欄として置く。全 description 省略時は空の説明領域を作らずコンセプトだけを表示する。ブロック地は `--text-dim`、文字は `--bg`。横は狭い画面で項目単位に縦積みする。
- **縮退先:** 通常の箇条書き。
...
- **受理条件:**
  ...
  - `orientation: "horizontal"` → steps 3〜6 件、`description` は 1〜2 行（各 30 字以内）。number モードでは横型も `title` を許可し必須とする。
  - **コンセプト必須契約**（enumeration と同一）: `blockContent: "number"` の全 step に `title` が必須。番号は識別子であり、`description` はコンセプトの代わりにならない。違反は `chevron_structure_violation`。
...
- **表現:** `label` または番号＋`title` だけをチェブロン図形内に置く。`description` がある場合、縦型は図形の右、横型は図形の下に図形外の説明欄として置く。全 description 省略時は空の説明領域を作らない。ループはコンセプト列だけに沿う左レール＋上向き矢頭で、レンダラ所有・`data-ve-from/to` なし。`loop: true` のとき、visually-hidden の戻り文も出力する。横は左に切り欠きのある矢羽で、狭い画面では項目単位に縦積みする。
...
ADOPTED: R1-3 — number モードの最小可視内容契約は、2026-07-12 の description-layout 設計で厳格化した。enumeration/chevron の全 item/step に `title` を必須とし、横チェブロンでも `title` を許可・必須化する。`description` はコンセプト外の任意補足であり、全有/全無規則を維持する。各契約節。
