...
- **Test fixtures:** the checker byte-compares fixed regions against `assets/skeleton.html`, so skeleton-embedding fixtures must be re-spliced (measured: 85 of 87 `scripts/tests/*.html` embed skeleton regions; `scripts/tests/tools/resplice.py` currently processes 83 and skips 4 via `KEEP_AS_IS`). `KEEP_AS_IS` entries whose fixed regions match the current skeleton must also be updated, while the two `*-fixed-region` fixtures preserve their intentional divergences; the `KEEP_AS_IS` set is revised accordingly.
