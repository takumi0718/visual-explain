...
- **Test fixtures:** the checker byte-compares fixed regions against `assets/skeleton.html`, so skeleton-embedding fixtures must be re-spliced (measured: 86 of 87 `scripts/tests/*.html` embed the skeleton's fixed CSS; only `compatibility-valid-fragment.html` does not). `resplice.py`'s default glob would destroy the two marker-less fixtures (`bad-title-missing.html`, `compatibility-valid-fragment.html`), so `KEEP_AS_IS` grows to six entries; the tool re-splices 81 fixtures plus the example, and the five KEEP_AS_IS files that embed skeleton CSS receive the same three CSS edits textually, preserving their intentional marker damage.
